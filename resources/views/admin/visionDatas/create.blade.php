...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.visionData.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.vision-datas.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('trial_type') ? 'is-invalid' : '' }}" name="trial_type" id="trial_type" required>
                    ...
                    @foreach(App\Models\VisionData::TRIAL_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('trial_type', 'birth') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('time_taken') ? 'is-invalid' : '' }}" type="text" name="time_taken" id="time_taken" value="{{ old('time_taken') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('file_type') ? 'is-invalid' : '' }}" name="file_type" id="file_type" required>
                    ...
                    @foreach(App\Models\VisionData::FILE_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('file_type', 'image') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
